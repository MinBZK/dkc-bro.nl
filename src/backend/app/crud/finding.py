...
def get_findings_and_rules(
    db: Session, skip: int = 0, limit: int = 100, filter_violations=False
) -> List:
    """
    ...
    """
    query = db.query(Finding, Rule).join(Rule).order_by(Finding.timestamp.desc())
    if filter_violations:
        query = query.filter(Finding.result == False)
...
def get_historical_results_previous_batches_by_ruleid(
    ...
    finding_results = (
        db.query(
            ...
            .filter(Finding.batch_id == batch_id, Finding.result == True)
            .label("pass"),
            func.count(Finding.result)
            .filter(Finding.batch_id == batch_id, Finding.result == False)
            .label("fail"),
        )
        .join(Rule)
