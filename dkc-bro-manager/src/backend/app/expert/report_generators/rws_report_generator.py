        ...
        if len(self.data) > 0:
            for row in reversed(self.data):
                for finding in row["findings"]:
                    if finding.OrganizationRule.importance < 2 or not finding.OrganizationRule.enabled:
                        continue
                    if f"{finding.OrganizationRule.rule_object_type}-{finding.OrganizationRule.rule_id}" not in rules:
                        rules[f"{finding.OrganizationRule.rule_object_type}-{finding.OrganizationRule.rule_id}"] = {
                            "gewicht": self.__translate_weight_to_dutch(
                                Importance(finding.OrganizationRule.importance).name
                            ),
                            "uitleg": finding.OrganizationRule.explanation,
                            "occurences": 0,
                            "violated_files": [],
                        }
                    rules[f"{finding.OrganizationRule.rule_object_type}-{finding.OrganizationRule.rule_id}"][
            ...
            [
                Image(
                    filename=os.path.join(
                        ...
                        (
                            "warning-white.png"
                            if finding.OrganizationRule.importance == 2
                            else "error-white.png"
                        ),
                    ),
                    width=10 * mm,
                ...
                Paragraph(f"{finding.OrganizationRule.rule_object_type}-{finding.OrganizationRule.rule_id}"),
                Paragraph(
    ...
    def getContent(self, contentBuilder):
        ...
        for document in self.data:
            ...
            for finding in [
                ...
                if not f.Finding.result and f.OrganizationRule.importance > 1
            ]:
