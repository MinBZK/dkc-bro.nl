...
class CPT0007A(superRule):
    ...
    __feedbackMessage = "Domein Kwaliteitsklasse (qualityClass) is gevuld met een niet geldige waarde: '%s'."
    ...
    def applyRule(self, doc):
        quality_class = doc.get_element_from_metadata("qualityClass")
        if quality_class in self.invalid_quality_classes:
            return self.getFeedbackMessage() % quality_class
        else:
            return None
