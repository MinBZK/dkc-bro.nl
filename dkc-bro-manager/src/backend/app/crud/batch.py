...
def get_existence_of_batch(db: Session, batch_id: str) -> bool:
    """
    Checks the database for the existence of a batch with the given id.
...
def create_batch_if_not_exists(
    db: Session, batch: schemas.batch.BatchCreate
) -> Batch | None:
    """
    ...
    """
    if get_existence_of_batch(db=db, batch_id=batch.id):
        return
...
def get_all_batch_ids(db: Session, org_id: int) -> List[str]:
    """
    Gets all batch ids from the database.

    Returns: List of batch ids.
    """
    return [batch.id for batch in db.query(Batch.id).filter(Batch.org_id == org_id).all()]


def get_batches(db: Session, org_id, skip: int = 0, limit: int = 100) -> List[Batch]:
    """
