...
async def get_findings(
    ...
    return crud.finding.get_findings_and_rules(
        db=db, org_id=org_id, skip=skip, limit=limit, filter_violations=True
    )
...
async def get_findings_per_document(
    ...
    documents = {}
    org_id = current_user.organization.id
    for result in crud.finding.get_findings_and_rules(db=db, org_id=org_id, skip=skip, limit=limit):
...
async def download_findings(
    ...
    return crud.finding.get_findings_and_rules_and_project(
        db=db, start_date=start_date, end_date=end_date, org_id=org_id
    )
...
async def get_findings_area_chart(
    ...
    """
    org_id = current_user.organization.id
    return crud.aggregate.get_finding_results_by_date(db, org_id)
