  ...
  <v-container>
    <v-row>
      <v-col cols="12" class="mb-4">
        ...
        <p v-else>
          Hier ziet u de kwaliteitsregels van Rijkswaterstaat die per brondocument gelden. Op deze pagina kunt
      ...
      <v-col cols="12">
        ...
        >
          <template v-slot:top v-if="$store.state.user.isAuthenticated">
            <v-toolbar flat>
              <v-toolbar-title>Regelbeheer</v-toolbar-title>
          ...
          <template v-slot:no-data>
            <v-alert type="info" class="ma-4" color="primary">
              Geen regels die op waarschuwing of fout staan.
            </v-alert>
          </template>
...
export default Vue.extend({
  ...
  methods: {
    getRules() {
      if (store.state.user.isAuthenticated) {
        axios
          .get(`${store.state.APIurl}/rule`, {
            headers: { Authorization: `Bearer ${store.state.user.token}` },
          })
          .then((response) => {
            this.rules = response.data;
            const presentTypes: string[] = [
              ...new Set(this.rules.map((rule: Rule) => rule.object_type)),
            ] as string[];
            this.documentTypes = ["Alle"].concat(presentTypes.sort());
          })
          .catch(() => {
            this.rules = [];
          });
        ...
        axios
          .get(`${store.state.APIurl}/rule-demo`, {
          })
          .then((response) => {
            this.rules = response.data;
            const presentTypes: string[] = [
