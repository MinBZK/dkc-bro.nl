...
class CPT0006A(superRule):
    ...
    __feedbackMessage = (
        "Domein Sondeernorm (cptStandard) is ingevuld met een ongeldige optie: '%s'."
    )
    ...
    def applyRule(self, doc):
        cpt_standard = doc.get_element_from_metadata("cptStandard")
        if cpt_standard in ["NEN5140", "NEN3680", "onbekend"]:
            return self.getFeedbackMessage() % cpt_standard
        else:
            return None
