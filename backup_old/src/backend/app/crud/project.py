...
def get_project_nrs(db: Session, include_inactive: bool = False) -> List[Project]:
    """
    ...
    if include_inactive:
        ...
        query = (
            db.query(Project)
            .filter((Project.environment == env) & (Project.active == True))
            .order_by(Project.project_nr)
        )
    return query.all()
...
def create_project_nrs(
    db: Session, project_id: int, project_name: str, source_holder: str, closed: bool
) -> str:
    active = not closed
    existing_project = (
        db.query(Project)
        .filter(
            (Project.project_nr == project_id) & (Project.project_name == project_name)
        )
        .first()
    )
    ...
    if existing_project and same_project_status:
        ...
        logging.info(
            f"Project number {project_id} with name: {project_name} and source holder {source_holder} is added to the database."
        )
        payload = {
