...
class ExpertRws(ExpertBase):
    ...
    def __register_rules(self) -> List[superRule]:
        ...
        for py_file in py_files:
            ...
            for name, obj in inspect.getmembers(module):
                if (
                    inspect.isclass(obj)
                    and issubclass(obj, superRule)
                    and name != "superRule"
                ):
                    rules.append(obj)
        return rules

    def __find_parser_for_document(self, document) -> Optional[XmlParser]:
        """
    ...
    def __validate_document(self, document_blob: str, document_name: str) -> List:
        ...
        """
        findings = []
        parser = self.__find_parser_for_document(document_blob)
